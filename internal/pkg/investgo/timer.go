	...
	for {
		...
		default:
			...
			case time.Now().Before(today.GetStartTime().AsTime()):
				t.client.Logger.Infof("%v ещё закрыт, ожидание старта %v", t.exchange, time.Until(today.GetStartTime().AsTime().Local()))
				if stop := t.waitUntil(ctxTimer, today.GetStartTime().AsTime().Local()); stop {
					return nil
				}
				t.events <- START
				t.client.Logger.Infof("начало торговой сессии, оставшееся время = %v", time.Until(today.GetEndTime().AsTime().Local()))
			...
			case time.Now().After(today.GetEndTime().AsTime().Local().Add(-t.cancelAhead)):
				...
				t.client.Logger.Infof("%v уже закрыт, ожидание следующего дня 1 час", t.exchange)
...
func (t *Timer) shutdown() {
	t.client.Logger.Infof("остановка таймера %v", t.exchange)
	close(t.events)
}
