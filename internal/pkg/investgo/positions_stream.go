		...
		default:
			...
			if err != nil {
				...
				case status.Code(err) == codes.Canceled:
					p.operationsClient.logger.Infof("остановка прослушивания позиций")
					return nil
				default:
					return err
				}
				...
				default:
					p.operationsClient.logger.Infof("информация из стрима позиций: %v", resp.String())
				}
			}
		}
	}
...
func (p *PositionsStream) restart(_ context.Context, attempt uint, err error) {
	p.operationsClient.logger.Infof("попытка перезапуска стрима позиций, ошибка = %v, попытка = %v", err.Error(), attempt)
}

func (p *PositionsStream) shutdown() {
	p.operationsClient.logger.Infof("закрытие стрима позиций")
	close(p.positions)
}
