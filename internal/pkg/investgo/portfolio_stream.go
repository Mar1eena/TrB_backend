		...
		default:
			...
			if err != nil {
				...
				case status.Code(err) == codes.Canceled:
					p.operationsClient.logger.Infof("остановка прослушивания портфелей")
					return nil
				default:
					return err
				}
				...
				default:
					p.operationsClient.logger.Infof("информация из стрима портфеля: %v", resp.String())
				}
			}
		}
	}
...
func (p *PortfolioStream) restart(_ context.Context, attempt uint, err error) {
	p.operationsClient.logger.Infof("попытка перезапуска стрима портфеля, ошибка = %v, попытка = %v", err.Error(), attempt)
}

func (p *PortfolioStream) shutdown() {
	p.operationsClient.logger.Infof("закрытие стрима портфеля")
	close(p.portfolios)
}
