"""Глобальный кэш оценок кандидатов поиска (Postgres search_eval_cache).

Ключ кэша фиксирует всё, от чего зависят метрики бэктеста: канонический хэш
StrategySpec, инструмент/интервал/период, доля периода (fidelity), параметры
брокера и версия движка. Смена любого из них => промах => пересчёт.
"""
...
class EvalCache:
    """Обёртка read-through / write-behind над pg.search_eval_cache.
