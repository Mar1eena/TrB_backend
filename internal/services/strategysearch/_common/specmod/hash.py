"""Канонический хэш StrategySearchSpec — совпадает с Go spechash.Hash64.
...
def spec_hash_u64(spec: spec_pb2.StrategySearchSpec) -> int:
    payload = spec.SerializeToString(deterministic=True)
    digest = hashlib.sha256(payload).digest()
    return int.from_bytes(digest[:8], "little", signed=False)


def spec_hash_signed(spec: spec_pb2.StrategySearchSpec) -> int:
