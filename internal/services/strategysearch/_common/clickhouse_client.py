...
def _parse_host_port(raw: str) -> tuple[str, int]:
    ...
    if port == 9000:
        log.debug(
            "Указан native-порт 9000, переключаем на HTTP-порт %s для clickhouse-connect",
            HTTP_PORT,
        )
        port = HTTP_PORT
...
def connect_with_retry() -> Client:
    ...
    retries = _env_int("STRATEGYSEARCH_CH_CONNECT_RETRIES", 30)
    backoff = _env_int("STRATEGYSEARCH_CH_CONNECT_BACKOFF_SEC", 2)
    attempt = 0
    while True:
        attempt += 1
        try:
