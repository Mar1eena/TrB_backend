...
Порт internal/services/strategy/_common/btcore.py: добавлены benchmark_df
(для QuantStats alpha/beta/information_ratio/r_squared) и checkpoint_interval_bars
(пост-фактум чекпойнты капитала для Optuna trial.report/should_prune).
"""
...
def _default_version() -> str:
    ...
    try:
        ...
        ov = "?"
    return f"bt{bt.__version__}+talib{tav}+optuna{ov}+ssengine1"


ENGINE_VERSION = os.environ.get("STRATEGYSEARCH_ENGINE_VERSION") or _default_version()


def run_backtest_inproc(
    spec, df: pd.DataFrame, config, precomputed: dict | None = None, *,
    with_indicators: bool = False, lean: bool = False,
    benchmark_df: pd.DataFrame | None = None, checkpoint_interval_bars: int = 0,
) -> dict[str, Any]:
    """Чистый прогон: без PG/CH/NATS. Возвращает {'metrics','equity','trades'[,'indicators','checkpoints']}.
    ...
    lean: только метрики (без кривой капитала и списка сделок) — для поиска,
    экономит сотни МБ на длинных сериях. Несовместимо с with_indicators.
    benchmark_df: свечи инструмента сравнения (BacktestConfig.benchmark_uid) —
    для alpha/beta/information_ratio/r_squared в BacktestMetrics.
    checkpoint_interval_bars: >0 => каждые N баров фиксировать капитал для
    пост-фактум прунинга Optuna (result['checkpoints']); 0 => не считать.
    """
    if df.empty:
    ...
    try:
        return _cerebro_once(spec, df, config, precomputed, runonce=True,
                             ...
                             benchmark_df=benchmark_df, checkpoint_interval_bars=checkpoint_interval_bars)
...
def _cerebro_once(
    ...
    an.attach(cerebro, lean=lean, checkpoint_interval_bars=checkpoint_interval_bars)

    results = cerebro.run()
    strat = results[0]
    result = an.extract(strat, cash, lean=lean, benchmark_df=benchmark_df)
    if with_indicators:
