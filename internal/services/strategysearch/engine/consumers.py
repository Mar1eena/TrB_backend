...
from strategysearch import backtest_pb2, search_pb2, strategysearch_pb2
from tasks_subjects import (
    CONSUMER_BACKTEST,
    CONSUMER_SEARCH,
    STREAM_STRATEGYSEARCH_BACKTEST,
    STREAM_STRATEGYSEARCH_TASKS,
    SUBJ_IMPORTANCE_REQUEST,
)
...
def _backtest_concurrency() -> int:
    raw = os.environ.get("STRATEGYSEARCH_BACKTEST_CONCURRENCY")
    try:
...
async def consume_backtest(js, ch_client: "Client", stop: asyncio.Event, *,
    ...
    psub = await bind(js, CONSUMER_BACKTEST, STREAM_STRATEGYSEARCH_BACKTEST)
    concurrency = _backtest_concurrency()
    log.info("backtest консьюмер привязан (%s/%s), параллельно до %d",
             STREAM_STRATEGYSEARCH_BACKTEST, CONSUMER_BACKTEST, concurrency)
    ...
    try:
        await loop(psub, stop, _handle_backtest, pool, nak_delay, len(clients))
    finally:
        for c in clients:
            if c is ch_client:
                continue
