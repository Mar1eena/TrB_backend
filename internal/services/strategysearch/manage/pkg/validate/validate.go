// Package validate — статическая проверка StrategySearchSpec до запуска движка.
...
func Spec(spec *strategysearchpb.StrategySearchSpec) (ok bool, issues []Issue) {
	...
	trees := []struct {
		name string
		expr *strategysearchpb.BoolExpr
	}{
	...
	if s := spec.GetSizing(); s != nil {
		...
		case *strategysearchpb.PositionSizing_PercentEquity:
			if s.GetPercentEquity() <= 0 || s.GetPercentEquity() > 1 {
				v.add("sizing.percent_equity", "ожидается доля в (0, 1]")
			}
		case *strategysearchpb.PositionSizing_RiskPerTrade:
...
func (c *checker) walk(expr *strategysearchpb.BoolExpr, path string, depth int) {
	if expr == nil {
		return
	}
	if depth > MaxDepth {
	...
	case *strategysearchpb.BoolExpr_Any:
	...
	case *strategysearchpb.BoolExpr_Negate:
		c.walk(n.Negate, path+".negate", depth+1)
	case *strategysearchpb.BoolExpr_Literal:
		// ок
	default:
...
func (c *checker) checkOperand(op *strategysearchpb.Operand, path string) {
	if op == nil {
	...
	case *strategysearchpb.Operand_IndicatorId:
		if !c.ids[n.IndicatorId] {
			c.add(path+".indicator_id", "неизвестный индикатор: "+n.IndicatorId)
		}
	case *strategysearchpb.Operand_Price:
	case *strategysearchpb.Operand_Constant:
	case *strategysearchpb.Operand_Arith:
