...
func (s *Server) GetBacktestResult(ctx context.Context, req *strategysearchpb.GetBacktestResultRequest) (*strategysearchpb.GetBacktestResultResponse, error) {
	...
	resp := &strategysearchpb.GetBacktestResultResponse{Run: run}
...
func (s *Server) readIndicatorSeries(ctx context.Context, runID string, maxPoints int) ([]*strategysearchpb.BacktestIndicatorSeries, error) {
	...
	if err := s.ch.QueryRow(ctx, `
		SELECT max(cnt) FROM (
			SELECT count() AS cnt
			FROM TrB_strategysearch.indicator_series FINAL
			WHERE run_id = ?
	...
	byKey := make(map[string]*strategysearchpb.BacktestIndicatorSeries)
	var order []string
	for rows.Next() {
		var (
		...
		ser := byKey[key]
		if ser == nil {
			ser = &strategysearchpb.BacktestIndicatorSeries{
				IndicatorId: indID,
				Indicator:   indicator,
				OutputKey:   outputKey,
				Overlay:     overlay == 1,
		...
		ser.Points = append(ser.Points, &strategysearchpb.BacktestIndicatorPoint{
	...
	out := make([]*strategysearchpb.BacktestIndicatorSeries, 0, len(order))
	for _, k := range order {
...
func (s *Server) readEquity(ctx context.Context, runID string, maxPoints int) ([]*strategysearchpb.EquityPoint, error) {
	// Даунсэмплинг: каждый n-й бар, если точек больше лимита.
	var count uint64
	if err := s.ch.QueryRow(ctx,
		`SELECT count() FROM TrB_strategysearch.equity_curve FINAL WHERE run_id = ?`, runID).Scan(&count); err != nil {
		return nil, err
	}
	stride := 1
	...
	rows, err := s.ch.Query(ctx, `
		...
		FROM (
			...
			FROM TrB_strategysearch.equity_curve FINAL
			WHERE run_id = ?
		)
		WHERE rn % ? = 0
	...
	out := make([]*strategysearchpb.EquityPoint, 0, maxPoints)
	for rows.Next() {
		var (
			t                                   time.Time
		...
		out = append(out, &strategysearchpb.EquityPoint{
			Time:          timestamppb.New(t),
			Equity:        equity,
			Cash:          cash,
...
func (s *Server) readTrades(ctx context.Context, runID string) ([]*strategysearchpb.TradeRecord, error) {
	rows, err := s.ch.Query(ctx, `
		...
		FROM TrB_strategysearch.trades FINAL
		WHERE run_id = ?
	...
	out := make([]*strategysearchpb.TradeRecord, 0)
	for rows.Next() {
		var (
			tradeID              uint32
			isLong               uint8
		...
		out = append(out, &strategysearchpb.TradeRecord{
			TradeId:     tradeID,
			IsLong:      isLong == 1,
