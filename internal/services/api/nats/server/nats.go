...
func (s *natsService) AddConsumer(context context.Context, req *nats_admin.Consumer) (*nats_admin.ConsumerInfos, error) {
	...
	if e != nil {
		s.js.L.Err(e).Str("stream", req.GetName()).Str("consumer", consumerName(req)).Msg("не удалось добавить консьюмер")
	} else {
		s.js.L.Info().Str("stream", req.GetName()).Str("consumer", consumerName(req)).Msg("консьюмер успешно добавлен")
	}
	return r, e
}
...
func (s *natsService) UpdateConsumer(context context.Context, req *nats_admin.Consumer) (*nats_admin.ConsumerInfos, error) {
	...
	if e != nil {
		s.js.L.Err(e).Str("stream", req.GetName()).Str("consumer", consumerName(req)).Msg("не удалось обновить консьюмер")
	} else {
		s.js.L.Info().Str("stream", req.GetName()).Str("consumer", consumerName(req)).Msg("консьюмер успешно обновлён")
	}
	return r, e
}
